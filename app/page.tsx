...
function RoadmapCard({ name, priceLabel, blurb, badge = "Roadmap", footnote, ctaLabel = "Coming soon" }) {
  return (
    <div className="relative flex flex-col justify-between rounded-2xl border border-gray-300 bg-gray-50 p-6 md:p-8 shadow-sm opacity-75">
      <div className="absolute -top-3 right-6 select-none rounded-full bg-gray-500 px-3 py-1 text-xs font-medium text-white">{badge}</div>
      <div>
        <h3 className="text-xl font-semibold tracking-tight text-gray-700">{name}</h3>
        <p className="mt-2 text-sm text-gray-500">{blurb}</p>
        {priceLabel && (
          <div className="mt-6 flex items-end gap-2">
            <span className="text-4xl font-bold tracking-tight text-gray-600">{priceLabel}</span>
          </div>
        )}
      </div>
      <a
        href="#"
        aria-disabled="true"
        className="mt-8 inline-flex items-center justify-center rounded-xl border border-gray-400 bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-600 pointer-events-none"
      >
        {ctaLabel}
      </a>
      <p className="mt-3 text-xs text-gray-400 min-h-5">{footnote || ' '}</p>
    </div>
  );
}
